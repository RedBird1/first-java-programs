...
/**
 * This application class will display a looped menu utilizing different cases for the menu options
 * The options are.
 * 1- Display Element Information 
 * 2- Display Particle Information 
 * 3-Get element with the highest boiling point. 
 * 4- Get the element with the lowest melting point. 
 * 5- Molecular mass Calculations. 
 * 6- Quit. Will quit the looped menue.
 * 
 * 
...
public class PeriodicTableApp
{
	...
	public static void main(String[] args)
	{
		...
		// Reads the file to get the database information into the array of element objects
		...
		// Menu loop. Will not loop if users choice is 6, the program will end.
		// If the userChoice is an invalid menu selection the the default case will be executed.
		// stating invalid choice. then user will be prompted by the menu again.
		while (userChoice != 6)
		{
			switch (userChoice)
			{
			...
			case 5:// Display the molecular mass calculations.
				...
				{
					// scans in line by line.
					String compoundSubstanceLine = input.nextLine();
					
					
					// delimiter that separates the line into an array split by
					// " ".
					String delimiter = " ";
					// Load delimited line into the Array.
					String[] compoundSubstanceArray = compoundSubstanceLine
							.split(delimiter);
					//replaces all 1s in the compound substance with " "(Space).
					compoundSubstanceLine=compoundSubstanceLine.replaceAll("1", " ");
					//replaces all of the spaces in the compound substance with ""(nothing)
					//removes spaces to properly display compound substance.
					compoundSubstanceLine=compoundSubstanceLine.replaceAll("\\s","");
					System.out.print(compoundSubstanceLine+"=");
					// pass the read array into the
					// calculateMolarMassOfCompoundSubstance() method.
					// Print calculated molar mass.
					System.out
							.println( periodicTable.calculateMolarMassOfCompoundSubstance(compoundSubstanceArray));
